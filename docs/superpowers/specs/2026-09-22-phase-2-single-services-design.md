...
| ---------------- | ---------------------------------------------------------------- | ----- | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| kopia            | `docker.io/kopia/kopia:0.23.1`, pinned                           | 51515 | config pool, cache fast, logs fast, repo fast (filesystem backend only) | Phase 2 ran the server as plain HTTP behind Traefik; Phase 3 reversed this because Kopia clients speak gRPC over HTTP/2: the server has its own self-signed TLS and Traefik re-encrypts (`route: {scheme: https, insecure_skip_verify: true}`). See the Phase 3 spec, section 4.                                    |
| jellyfin         | `lscr.io/linuxserver/jellyfin:latest`, auto                      | 8096  | config pool                                                             | bind media ro; devices `/dev/dri`; groups render, video; **no `UserNS=keep-id`** — the s6 image's init needs `CAP_SETGID`, so it runs under the default rootless mapping, and the media tree needs `o+rX` because the app process loses its groups either way; `JELLYFIN_PublishedServerUrl` from `service.config`. |
...
| paperless-upload | `.build` from `config/build/Containerfile` in the service folder | none  | none                                                                    | bind scans rw; `PAPERLESS_URL` env from `service.config` pointing at `paperless.<domain>`; runs as image user `bun` (uid 1000) with keep-id; `HealthCmd=test -d /data/in`. Uploads fail until Phase 3 delivers Paperless.                                                                                           |
