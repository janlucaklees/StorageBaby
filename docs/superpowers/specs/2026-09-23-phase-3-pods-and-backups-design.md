...
**Amendment (task 6b).** "`AddHost=kopia.<domain>:host-gateway` on the pod" above was right about the mechanism and too narrow about the scope, and it is no longer a template's line. Measured on the test VM (podman 6.1.2, pasta 2026_07_28): from inside a container, pod or not, a connect to the host's own LAN address on 443 is **refused** — pasta carries that address on the container's interface, so the connection terminates in the container — and only `169.254.1.2` (`host-gateway`) reaches Traefik. So this is not "harmless where DNS exists": where DNS exists it is _required_, because DNS answers with the unreachable address. The `service` role now renders `AddHost=<fqdn>:host-gateway` for every route name placed on the host, for every placed service, into a Quadlet drop-in (`<name>.pod.d/10-storagebaby-hosts.conf`, or `<stem>.container.d/` for a service without a pod — podman refuses `--add-host` on a container that joins a pod). Templates write no such line, and a static test enforces it. `AddHost=immich-machine-learning:127.0.0.1` is unaffected: it aliases a compose service name to the pod's loopback, not to the host.
...
Server: the kopia service gains `host_secrets` entries `client_<service>: kopia-clients.<service>` for every service with a backup, and the start script registers or updates each `<service>@<host>` user from `/run/secrets/client_*` before starting the server (repository-level command, no server needed).

TLS, revised in Task 3: Kopia's repository protocol is gRPC over HTTP/2. Traefik terminating TLS and forwarding plain HTTP to a server started with `--insecure` downgrades the call and it never completes, and Kopia's insecure listener does not speak h2c. So the Phase 2 decision is reversed: the kopia server generates its own self-signed certificate into its config volume on first start and listens on `https://0.0.0.0:51515`; Traefik re-encrypts to it with `insecureSkipVerify`. Routes therefore gain optional `scheme: https` and `insecure_skip_verify: true` (on `routes[]` entries and the `route:` shorthand), rendered as a per-route `serversTransport`. Clients still see Traefik's certificate: Let's Encrypt on storagebaby, and on `acme: false` hosts a self-signed default certificate that `host_base` generates once and installs as Traefik's default TLS store entry, so the sidecar's fingerprint pin survives Traefik restarts. Clients: the role-generated sidecar connects to `https://kopia.<domain>` (LE certificate on storagebaby; on test hosts, `acme: false`, it pins the server certificate fingerprint fetched at start), sets the policy from `backup`, and runs `kopia server start --no-ui --insecure --address http://127.0.0.1:51516` so the scheduler executes the snapshots. Verification on the test VM: trigger a snapshot from a sidecar and list it on the server.
